...
- [x] 6.3 手动冒烟测试（需 GARMIN_EMAIL/GARMIN_PASSWORD 环境变量）：调用 create_workout 创建一个简单跑步课（warmup→interval→cooldown），确认返回 workoutId；调用 schedule_workout 排程；调用 delete_workout 清理

## 7. 文档

- [x] 7.1 更新 README（若列有 tool 清单）：补充 create_workout / delete_workout / schedule_workout 三个 tool 及入参说明
