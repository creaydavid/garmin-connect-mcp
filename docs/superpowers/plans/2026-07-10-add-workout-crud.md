...
- [ ] **Step 1: 创建 `src/dtos/workout.dto.ts`，定义 StepInput type + schema + CreateWorkout/DeleteWorkout/ScheduleWorkout DTO**
...
- [ ] **Step 2: 在 `src/dtos/index.ts` 添加 barrel 导出**
...
- [ ] **Step 1: 在 `garmin.client.ts` 追加 import**
...
- [ ] **Step 2: 在 `todayString` 函数之前插入 4 个私有组装函数**
...
- [ ] **Step 1: 在 `removeGearFromActivity` 方法之后追加 3 个方法**
...
- [ ] **Step 1: 创建 `src/tools/workout.tools.ts`**
...
- [ ] **Step 2: 在 `src/tools/index.ts` 添加 barrel 导出**
...
- [ ] **Step 3: 在 `src/index.ts` import 并调用 registerWorkoutTools**
...
- [ ] **Step 1: 构建并启动 MCP server 验证 tool 注册**
...
- [ ] **Step 2: create_workout 冒烟 — 创建含 heart_rate target 的间歇跑**
...
- [ ] **Step 3: schedule_workout 冒烟 — 排程到明天**
...
- [ ] **Step 1: 在 README.md 的 "Profile & Devices" section 之后、"Authentication" section 之前插入 Workout Tools section**
